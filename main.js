...
projectTitle.textContent = "Virtual keyboard";
operationSystem.textContent = "The task is made using Mac.";
language.textContent =
  "To use another language use combination 'ctrl + shift'.";
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.code === "Tab") {
    ...
    const newText =
      text.slice(0, cursorPosition) + spaces + text.slice(cursorPosition);
  ...
  if (
    !specialKeys.includes(event.code) &&
    document.querySelector(`.${event.code}`) !== null
  ) {
    let character = document.querySelector(`.${event.code}`).innerHTML;
